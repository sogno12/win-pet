...
class PersonaBuilder:
    ...
    BASE_INSTRUCTION = (
        ...
        "- 사용자가 메모장/계산기를 켜달라거나 메모/기록/저장을 원하거나, PC 제어(화면 잠금, 앱 실행, 볼륨 조절, 구글/유튜브 검색 등)나 정보 조회를 요청하면 "
        "말로만 '(메모장을 실행합니다)', '잠시만 기다려주세요' 같은 시늉 대사를 출력하지 말고, 반드시 해당 함수 도구(launch_app, lock_pc, get_weather 등)만을 즉시 호출해라.\n"
