...
이미지 크기가 가로 2816 px, 세로 1536 px 일때.
중앙 기준으로 정확하게 좌우를 똑같이 잘라내어 1536 x 1536 정사각형을 만들려면, 아래 수서대로 수치를 입력하고 기능을 실행하시면 됩니다. 픽픽의 특성을 활용한 가장 쉽고 완벽한 방법입니다.

## 중앙 기준 자르기 실행 순서
1. 1차 자르기 (우측 잘라내기)

* 올려주신 창에서 가로(Width) 입력창에 2176을 입력하고 [확인]을 누릅니다.
* 원리: 전체 2816에서 잘라내야 할 총 너비인 1280의 절반(640px)만큼인 우측을 먼저 잘라내는 과정입니다.

2. 이미지 좌우 반전하기

* 메인 화면 상단 메뉴에서 [홈] 탭 ➔ [회전] ➔ [수평 대칭 이동]을 클릭합니다.
* 이미지의 왼쪽과 오른쪽이 반대로 뒤집힙니다.

3. 2차 자르기 (나머지 한쪽 잘라내기)

* 다시 단축키 Ctrl + Alt + C를 눌러 캔버스 크기 변경 창을 엽니다.
* 이번에는 가로(Width) 입력창에 세로 크기와 동일한 1536을 입력하고 [확인]을 누릅니다.

4. 원래대로 되돌리기

* 다시 상단 메뉴에서 [회전] ➔ [수평 대칭 이동]을 클릭하여 이미지를 원래 방향으로 돌려놓습니다.
